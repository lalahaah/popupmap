...
export function Sidebar() {
  ...
  return (
    <aside className="desktop-sidebar w-[400px] h-full border-r-2 border-[var(--ink)] bg-[var(--paper)] flex flex-col z-20">
      {/* Header */}
      <div className="p-5 border-b-2 border-[var(--ink)]">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="display text-2xl tracking-tight">POPUP MAP</h1>
            <p className="text-xs font-bold mt-1 text-neutral-500">지금 진짜 열려있는 팝업만</p>
          </div>
          <div className="stub px-3 py-2 text-xs mono font-bold">D+0</div>
        </div>

        {/* status banner */}
        <div className="mt-4 border-2 border-[var(--ink)] bg-[var(--yellow)] px-4 py-3 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold">오늘 새로 오픈</p>
            <p className="display text-lg leading-none mt-1">{newCount}곳</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold">이번주 마감임박</p>
            <p className="display text-lg leading-none mt-1 text-[var(--red)]">{closingSoonCount}곳</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 pt-4">
        <div className="flex items-center gap-2 border-2 border-[var(--ink)] px-3 py-2.5">
      ...
      {/* Sort tabs */}
      <div className="px-5 pt-4 pb-2 flex items-center justify-between border-b-2 border-[var(--ink)]">
        <div className="flex gap-4 text-sm font-bold">
          <button className="pb-2 border-b-[3px] border-[var(--red)]">마감임박순</button>
          <button className="pb-2 text-neutral-400">신규순</button>
          <button className="pb-2 text-neutral-400">인기순</button>
        </div>
        <span className="text-xs mono font-bold text-neutral-500">지도 내 {popups.length}곳</span>
      </div>
      ...
      <div className="p-5 border-t-2 border-[var(--ink)]">
        <button className="w-full py-3 bg-[var(--red)] text-white font-bold border-2 border-[var(--ink)] shadow-[4px_4px_0_var(--ink)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0_var(--ink)] transition-all">
          + 팝업 제보하기
        </button>
      </div>
    </aside>
